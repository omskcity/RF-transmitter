...
#define			TimeOutReceivePacket	1000	// Тайм-аут при приёме по USART. В мсек.
...
// Светодиод 2
#define LED2_ON		GPIO_SetBits(GPIOB,GPIO_Pin_5);
#define LED2_OFF	GPIO_ResetBits(GPIOB,GPIO_Pin_5);
...
void putstr(unsigned char *a, unsigned int Len ) {
	LED2_ON;								// включаем светодиод UART (2)
	while (Len--) {							// Отправляем буфер побайтно
		while( !USART_GetFlagStatus(USART1,USART_FLAG_TXE));
		USART_SendData(USART1,*a);
		a++;
	}
	LED2_OFF;								// выключаем светодиод UART (2)
}
...
void USART1_IRQHandler(void) {
	if (USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) {
		if( sTXBuffer.uiWR == 0 ) LED2_ON;						// Первый символ. Включаем светодиод UART (2)
